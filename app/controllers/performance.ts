...
export async function search(searchConditions: ISearchConditions): Promise<ISearchResult> {
    ...
    if (searchConditions.wheelchair) {
        ...
        const reservations: any[] = await ttts.Models.Reservation.find(conditionsWheelchair, 'performance').exec();
    ...
    const getWheelchairAvailable = async (pId: string) => {
        ...
        let reservationAvailable: number = await ttts.Models.Reservation.find(conditionsAvailable).count().exec();
