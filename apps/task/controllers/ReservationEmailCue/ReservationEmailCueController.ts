        ...
        Models.ReservationEmailCue.findOneAndUpdate(
            ...
            (err, cue) => {
                ...
                Util.getReservationLogger(cue.get('payment_no'), (getReservationLoggerErr, loggerOfReservation) => {
                    if (getReservationLoggerErr) return this.next(getReservationLoggerErr, cue, this.logger, cb);
                    ...
                    Models.Reservation.find(
                        ...
                        (findReservationErr, reservations) => {
                            ...
                            if (findReservationErr) return this.next(findReservationErr, cue, loggerOfReservation, cb);
                            if (reservations.length === 0) return this.next(null, cue, loggerOfReservation, cb);
                            ...
                            if (!to) return this.next(null, cue, loggerOfReservation, cb);
                            ...
                            template.render(locals, (renderErr, result) => {
                                ...
                                if (renderErr) return this.next(new Error('failed in rendering an email.'), cue, loggerOfReservation, cb);
